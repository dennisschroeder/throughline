...
func TestCoordinationAndAuthorityVerticalSlice(t *testing.T) {
	...
	claimResult, err := app.UnwrapMutation(service.ClaimWorkItem(ctx, claimCommand))
	if err != nil {
		t.Fatal(err)
	}
	replayedClaim, err := app.UnwrapMutation(service.ClaimWorkItem(ctx, claimCommand))
	if err != nil {
		t.Fatal(err)
	}
	...
	progress, err := app.UnwrapMutation(service.AppendProgress(ctx, app.AppendProgressCommand{
	...
	evidence, err := app.UnwrapMutation(service.AttachArtifact(ctx, app.AttachArtifactCommand{
	...
	actionResult, err := app.UnwrapMutation(service.ProposeExternalAction(ctx, app.ProposeExternalActionCommand{
	...
	resolution, err := app.UnwrapMutation(service.ResolveExternalActionApproval(ctx, app.ResolveExternalActionApprovalCommand{
	...
	_, err = app.UnwrapMutation(service.StartExternalActionExecution(ctx, app.StartExternalActionExecutionCommand{
	...
	completed, err := app.UnwrapMutation(service.CompleteExternalActionExecution(ctx, app.CompleteExternalActionExecutionCommand{
	...
	for _, target := range []work.ExecutionStatus{work.StatusReview, work.StatusDone} {
		item, err = app.UnwrapMutation(service.TransitionWorkItem(ctx, app.TransitionWorkItemCommand{
	...
	staleAction, err := app.UnwrapMutation(service.ProposeExternalAction(ctx, app.ProposeExternalActionCommand{WorkItemID: item.ID, ActorID: "agent:researcher", ExpectedVersion: item.Version, IdempotencyKey: "propose-stale-grant", Required: false, Title: "Publish a revised dossier", Rationale: "Verifies revision-bound authority.", Subject: staleSubject}))
	if err != nil {
		t.Fatal(err)
	}
	item.Version++
	staleApproval, err := app.UnwrapMutation(service.RequestExternalActionApproval(ctx, app.RequestExternalActionApprovalCommand{ActionID: staleAction.Action.ID, ActorID: "agent:researcher", ExpectedActionVersion: staleAction.Action.Version, ExpectedSubjectHash: staleAction.Revision.AuthorizationSubjectHash, IdempotencyKey: "request-stale-grant", ApprovedForActorID: "agent:researcher", Constraints: json.RawMessage(`{"audience":"internal"}`), Request: "Authorize this exact dossier publication."}))
	if err != nil {
		t.Fatal(err)
	}
	staleResolution, err := app.UnwrapMutation(service.ResolveExternalActionApproval(ctx, app.ResolveExternalActionApprovalCommand{ApprovalID: staleApproval.ID, ActorID: "human:owner", ExpectedActionVersion: staleAction.Action.Version, IdempotencyKey: "resolve-stale-grant", Decision: authority.ApprovalApproved, Rationale: "The first payload is bounded."}))
	if err != nil {
		t.Fatal(err)
	}
	staleAction, err = app.UnwrapMutation(service.ReviseExternalAction(ctx, app.ReviseExternalActionCommand{ActionID: staleResolution.Action.ID, ActorID: "agent:researcher", ExpectedActionVersion: staleResolution.Action.Version, ExpectedWorkItemVersion: item.Version, IdempotencyKey: "revise-stale-grant", Subject: json.RawMessage(`{"action_type":"knowledge.publish","target":{"collection":"research-library","version":"v2"},"arguments":[],"scope":{"workspace":"local"},"permissions":["knowledge.write"],"credential_requirements":[],"constraints":{"audience":"internal"}}`)}))
	if err != nil {
		t.Fatal(err)
	}
	item.Version++
	...
	_, err = app.UnwrapMutation(service.AppendProgress(ctx, app.AppendProgressCommand{
...
func createReadyResearchItem(t *testing.T, ctx context.Context, service *app.Service) work.WorkItem {
	...
	if _, err := app.UnwrapMutation(service.ReviewPlan(ctx, app.ReviewPlanCommand{PlanID: plan.Plan.ID, ReviewerActorID: "human:owner", IdempotencyKey: "review-plan-coordination", Decision: work.PlanApproved, Reason: "The plan is ready.", ExpectedVersion: 1})); err != nil {
		t.Fatal(err)
	}
...
func TestConcurrentAgentsCannotBothClaimWorkItem(t *testing.T) {
	...
	for _, command := range []app.RegisterActorCommand{
		...
		if _, err := app.UnwrapMutation(serviceOne.RegisterActor(ctx, command)); err != nil {
			t.Fatal(err)
		}
	}
	item := createReadyResearchItem(t, ctx, serviceOne)
	for _, actorID := range []string{"agent:one", "agent:two"} {
		if _, err := app.UnwrapMutation(serviceOne.AssignActorCapability(ctx, app.AssignActorCapabilityCommand{
	...
	for index, service := range []*app.Service{serviceOne, serviceTwo} {
		...
		go func(service *app.Service, actorID string) {
			<-start
			_, err := app.UnwrapMutation(service.ClaimWorkItem(ctx, app.ClaimWorkItemCommand{
...
func TestClaimRequiresNoOpenBlockerAndMatchingExecutionApproval(t *testing.T) {
	...
	item := createReadyResearchItem(t, ctx, service)
	if _, err := app.UnwrapMutation(service.AssignActorCapability(ctx, app.AssignActorCapabilityCommand{ActorID: "agent:researcher", Capability: "research", Description: "Can conduct source research.", GrantedBy: "human:owner", IdempotencyKey: "gates-capability"})); err != nil {
		t.Fatal(err)
	}
	if _, err := app.UnwrapMutation(service.AskQuestion(ctx, app.AskQuestionCommand{ObjectiveID: item.ObjectiveID, WorkItemID: item.ID, ActorID: "agent:researcher", IdempotencyKey: "ask-question-coordination", Question: "Is the source scope complete?"})); err != nil {
		t.Fatal(err)
	}
	_, err = app.UnwrapMutation(service.ClaimWorkItem(ctx, app.ClaimWorkItemCommand{WorkItemID: item.ID, ActorID: "agent:researcher", ExpectedVersion: item.Version, IdempotencyKey: "claim-blocked", LeaseDuration: time.Hour, TransitionToInProgress: true}))
	...
	assertReadyKeys(t, ready, item.Key)
	if _, err := app.UnwrapMutation(service.ClaimWorkItem(ctx, app.ClaimWorkItemCommand{WorkItemID: item.ID, ActorID: "agent:researcher", ExpectedVersion: item.Version, IdempotencyKey: "claim-approved", LeaseDuration: time.Hour, TransitionToInProgress: true})); err != nil {
		t.Fatal(err)
	}
}
