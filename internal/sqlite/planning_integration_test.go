...
func TestIntentAndPlanningVerticalSlice(t *testing.T) {
	...
	for _, command := range []app.RecordContextCommand{
		{ObjectiveID: objective.ID, ActorID: "agent:planner", Kind: work.ContextRequirement, Title: "Source-linked claims", Body: "Every material claim names its source.", Status: work.ContextAccepted},
		{ObjectiveID: objective.ID, ActorID: "human:sponsor", Kind: work.ContextConstraint, Title: "No external installation", Body: "This milestone records installation work but performs no side effect.", Status: work.ContextAccepted},
		{ObjectiveID: objective.ID, ActorID: "agent:planner", Kind: work.ContextAssumption, Title: "Three methods are enough", Body: "Validate during research.", Status: work.ContextUntested, Confidence: "medium"},
		{ObjectiveID: objective.ID, ActorID: "agent:researcher", Kind: work.ContextFinding, Title: "Provenance rubrics are reusable", Body: "A rubric can be applied across dossiers.", Status: work.ContextRecorded, SourceURI: "https://example.test/provenance"},
		{ObjectiveID: objective.ID, ActorID: "human:sponsor", Kind: work.ContextSuccessMetric, Title: "Independent recovery", Body: "A new agent can recover the approved intent and plan.", Status: work.ContextAccepted},
	} {
	...
	question, err := service.AskQuestion(ctx, app.AskQuestionCommand{
		ObjectiveID: objective.ID, ActorID: "agent:planner", Question: "Which audience owns final review?", RequiresHumanAttention: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := service.AnswerQuestion(ctx, app.AnswerQuestionCommand{
		QuestionID: question.ID, ActorID: "human:sponsor", Answer: "The research operations lead.", ExpectedVersion: 1,
	}); err != nil {
		t.Fatal(err)
	}
	waivedQuestion, err := service.AskQuestion(ctx, app.AskQuestionCommand{
		ObjectiveID: objective.ID, ActorID: "agent:planner", Question: "Should the skill include raw transcripts?",
	})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := service.WaiveQuestion(ctx, app.WaiveQuestionCommand{
		QuestionID: waivedQuestion.ID, ActorID: "human:sponsor", Reason: "Raw transcripts are outside the output contract.", ExpectedVersion: 1,
	}); err != nil {
		t.Fatal(err)
	}
	decision, err := service.RecordDecision(ctx, app.RecordDecisionCommand{
		ObjectiveID: objective.ID, ActorID: "human:sponsor", Title: "Use a reusable rubric", Decision: "Ship the provenance rubric with the skill.", Rationale: "It makes later reviews consistent.", Alternatives: []string{"Keep the rubric in chat"},
	})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := service.RecordDecision(ctx, app.RecordDecisionCommand{
		ObjectiveID: objective.ID, ActorID: "human:sponsor", Title: "Use a reusable rubric and uncertainty scale",
	...
	proposedProfile, err := service.ProposeOutputProfile(ctx, app.ProposeOutputProfileCommand{
		ActorID: "agent:designer", Name: "evidence_map", Version: 1, Description: "A claim-to-source evidence map.",
	...
	if _, err := service.ReviewOutputProfile(ctx, app.ReviewOutputProfileCommand{
		ProfileID: proposedProfile.ID, ReviewerActorID: "human:sponsor", Decision: output.ProfileActive, Reason: "The contract is domain-neutral and reviewable.",
	}); err != nil {
		t.Fatal(err)
	}
	if _, err := service.ProposeOutputProfile(ctx, app.ProposeOutputProfileCommand{
		ActorID: "agent:designer", Name: "evidence_map", Version: 2, Description: "Missing predecessor.",
	...
	profileV2, err := service.ProposeOutputProfile(ctx, app.ProposeOutputProfileCommand{
		ActorID: "agent:designer", Name: "evidence_map", Version: 2, Description: "An evidence map with explicit uncertainty.", SupersedesID: proposedProfile.ID,
	...
	if _, err := service.ReviewOutputProfile(ctx, app.ReviewOutputProfileCommand{
		ProfileID: profileV2.ID, ReviewerActorID: "human:sponsor", Decision: output.ProfileActive, Reason: "The successor makes uncertainty explicit.",
	}); err != nil {
		t.Fatal(err)
	}
	baseRubric, err := service.ProposeOutputProfile(ctx, app.ProposeOutputProfileCommand{
		ActorID: "agent:designer", Name: "review_rubric", Version: 1, Description: "A reviewed scoring rubric.",
	...
	if _, err := service.ReviewOutputProfile(ctx, app.ReviewOutputProfileCommand{
		ProfileID: baseRubric.ID, ReviewerActorID: "human:sponsor", Decision: output.ProfileActive, Reason: "The base rubric is usable.",
	}); err != nil {
		t.Fatal(err)
	}
	rejectedRubric, err := service.ProposeOutputProfile(ctx, app.ProposeOutputProfileCommand{
		ActorID: "agent:designer", Name: "review_rubric", Version: 2, Description: "A rejected rubric revision.", SupersedesID: baseRubric.ID,
	...
	if _, err := service.ReviewOutputProfile(ctx, app.ReviewOutputProfileCommand{
		ProfileID: rejectedRubric.ID, ReviewerActorID: "human:sponsor", Decision: output.ProfileRejected, Reason: "The weights were underspecified.",
	}); err != nil {
		t.Fatal(err)
	}
	replacementRubric, err := service.ProposeOutputProfile(ctx, app.ProposeOutputProfileCommand{
		ActorID: "agent:designer", Name: "review_rubric", Version: 3, Description: "A corrected rubric revision.", SupersedesID: baseRubric.ID,
	...
	if _, err := service.ReviewOutputProfile(ctx, app.ReviewOutputProfileCommand{
		ProfileID: replacementRubric.ID, ReviewerActorID: "human:sponsor", Decision: output.ProfileActive, Reason: "The corrected successor is complete.",
	}); err != nil {
		t.Fatal(err)
	}
	if _, err := service.ProposePlan(ctx, app.ProposePlanCommand{
	...
	if _, err := service.TransitionObjective(ctx, app.TransitionObjectiveCommand{
		ObjectiveID: objective.ID, TargetPhase: work.ObjectiveExecution, ActorID: "human:sponsor", Reason: "Start approved work.", ExpectedVersion: 1,
	}); err == nil {
	...
	if _, err := service.ReviewPlan(ctx, app.ReviewPlanCommand{
		PlanID: planContext.Plan.ID, ReviewerActorID: "human:sponsor", Decision: work.PlanApproved, Reason: "The scope, capabilities, and outputs are explicit.", ExpectedVersion: 1,
	}); err != nil {
		t.Fatal(err)
	}
	if _, err := service.TransitionObjective(ctx, app.TransitionObjectiveCommand{
	...
	replacement, err := service.ProposePlan(ctx, app.ProposePlanCommand{
		ObjectiveID: objective.ID, ActorID: "agent:planner", Title: "Source-auditing skill plan v2", Summary: "Refine the approved plan.", Revision: 2,
		Items: []app.ProposedWorkItem{{
	...
	if _, err := service.ReviewPlan(ctx, app.ReviewPlanCommand{
		PlanID: replacement.Plan.ID, ReviewerActorID: "human:sponsor", Decision: work.PlanApproved, Reason: "The new revision replaces the earlier scope.", ExpectedVersion: 1,
	}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := service.RecordContext(ctx, app.RecordContextCommand{
		ObjectiveID: otherObjective.ID, WorkItemID: itemIDs["WG-SKILL"], ActorID: "agent:planner",
	...
	if _, err := service.AskQuestion(ctx, app.AskQuestionCommand{
		ObjectiveID: otherObjective.ID, WorkItemID: itemIDs["WG-SKILL"], ActorID: "agent:planner", Question: "Cross-objective question?",
	}); err == nil {
		t.Fatal("expected cross-objective question to be rejected")
	}
	if _, err := service.RecordDecision(ctx, app.RecordDecisionCommand{
		ObjectiveID: otherObjective.ID, WorkItemID: itemIDs["WG-SKILL"], ActorID: "human:sponsor", Title: "Cross-objective decision", Decision: "Reject the invalid edge.",
	}); err == nil {
